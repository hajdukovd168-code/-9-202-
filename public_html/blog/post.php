...
<div class="container">
    <article class="post-full">
        ...
        <div class="post-meta"><div class="like-section" data-post-id="<?= $post['id'] ?>">
    <?php 
    $user_liked = false;
    ...
    $like_cnt = $pdo->prepare("SELECT COUNT(*) FROM likes WHERE post_id = ?");
    $like_cnt->execute([$post['id']]);
    $likes_total = $like_cnt->fetchColumn();
    ?>
    <button class="like-btn <?= $user_liked ? 'liked' : '' ?>" data-id="<?= $post['id'] ?>">
        ❤️ <span class="like-count"><?= $likes_total ?></span>
    </button>
</div>Автор: <?= htmlspecialchars($post['username']) ?> | Дата: <?= date('d.m.Y H:i', strtotime($post['created_at'])) ?></div>
